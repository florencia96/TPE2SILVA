...
class ApiComentarioController{
    ...
    public function insertComentario($params = null) {
        ...
        if (empty($comentario->comentario) || empty($comentario->nota) || empty($comentario->id_viaje)) {
            $this->view->response("Complete los datos", 400);
        } else {
            $id = $this->model->insert($comentario->comentario, $comentario->nota, $comentario->id_viaje);
            $comentario = $this->model->get($id);
